...
;;; helper functions
  ...
  (define (above-below-handler depth w mode)
    ...
    (require 'sawfish.wm.state.maximize)
    (let ()
      (define (set-unmaximized-depth w d)
	(if (window-maximized-fullscreen-p w)
	    (window-put w 'unmaximized-depth d)
	  (set-window-depth w d)))
      (define (unmaximized-depth w)
	(if (window-maximized-fullscreen-p w)
	    (window-get w 'unmaximized-depth)
	  (window-depth w)))
      (case mode
	((init)
	 (window-put w 'depth depth))
	((add remove)
	 (set-unmaximized-depth w (if (eq mode 'add) depth 0)))
	((toggle)
	 (set-unmaximized-depth w (if (= (unmaximized-depth w) depth) 0 depth)))
	((get)
	 (= (unmaximized-depth w) depth)))))
